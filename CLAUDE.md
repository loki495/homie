...
- **`ResolveDemoDatabase`** — homie has no per-user data model at all, so concurrent
  ...
  gets a `demo_instance_id` cookie (deliberately its own cookie, not Laravel's session
  ID — avoids any session-driver ordering issues) that maps to a private copy of
  `config('homie.demo_db_template_path')`, copied into
  `config('homie.demo_db_storage_path')/{id}.sqlite` on first visit. After pointing
  `database.connections.sqlite.database` at that file, it also calls `DB::purge('sqlite')`
  — a bare `config()` change is silently ineffective if the 'sqlite' connection was
  already resolved earlier in the same process (a long-running worker/Octane request,
...
**Testing this required opting out of `RefreshDatabase`'s shared-connection assumption.**
`tests/Feature/DemoModeTest.php` and `tests/Unit/Http/Middleware/ResolveDemoDatabaseTest.php`
exercise the real `DB::purge('sqlite')` reconnect above, which conflicts with
`RefreshDatabase` (active suite-wide, see `tests/Pest.php`): it begins each test's
transaction on one specific shared in-memory PDO tracked in
`RefreshDatabaseState::$inMemoryConnections`, and expects to roll back *that exact* PDO
at teardown — but once the middleware repoints/purges 'sqlite' mid-request, that name
resolves to a different connection object by teardown time, so Laravel's own rollback
callback rolls back the wrong object, leaves the real shared PDO's transaction dangling
open, and (seeing what looks like an already-rolled-back connection) marks the database
as unmigrated — corrupting every later test in the whole suite with `cannot VACUUM
from within a transaction` / `table "migrations" already exists` errors. Both test
files register their own `$this->beforeApplicationDestroyed()` callback in `beforeEach()`
(runs after RefreshDatabase's own, since it was registered later) that rolls back the
real shared PDO directly and confirms `RefreshDatabaseState::$migrated = true`. If
`DB::purge()`/reconnecting the default connection is ever needed in another
RefreshDatabase-backed test, this is the pattern to reuse — Laravel has no supported
public API to reassign a connection's PDO back onto its original transaction-tracking
state, so going straight at the raw PDO is deliberate, not a workaround-of-convenience.

Requires `SESSION_DRIVER=file` (or `cookie`) on any real demo deployment — the default
`database` driver would create a chicken-and-egg problem (need the DB to find the
session, need the session to pick the DB) since `StartSession` runs before
`ResolveDemoDatabase` in the `web` group.
